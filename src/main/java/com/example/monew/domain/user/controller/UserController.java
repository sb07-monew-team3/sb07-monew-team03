...
public class UserController {
    ...
    @PostMapping()
    ResponseEntity<UserDto> registerUser(@RequestBody UserRegisterRequest request){
        UserDto user = userService.createUser(request);
        return new ResponseEntity<UserDto>(user, HttpStatus.CREATED);
    }
    @PostMapping("/login")
    ResponseEntity<UserDto> loginUser(@RequestBody UserLoginRequest request){
    ...
    @PatchMapping("/{userId}")
    ResponseEntity<UserDto> updateUser(@RequestBody UserUpdateRequest request, @PathVariable("userId") UUID userId){
