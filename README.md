...
### 🟩 김진우
**관심사 도메인 전체 설계 및 구현**
- 관심사 CRUD (등록 / 수정 / 삭제 / 조회)
- Levenshtein Distance 알고리즘을 통한 중복 관심사 검증 (80% 유사도 임계값)
- 키워드 관리 (다중 키워드 등록 및 수정)
...
### 🟩 조성만
**뉴스 기사 관리 API**
- 뉴스 기사 수집 (오픈 API, RSS 활용)
    - 1시간 마다 배치 단위로 수행
- 뉴스 기사 RUD (조회 / 수정 / 삭제(논리, 물리))
- 뉴스 기사 백업 및 복구 (AWS S3 버킷 활용)
    - 하루마다 1일전 수집된 뉴스를 배치 단위로 백업
...
**로그 모니터링 구현**
 - 운영 서버 로그 S3에 저장
 - 요청마다 요청 url,엔드 포인트 , 요청 사용자 ID 로깅 구현
...
- 확인한 알림 중 1주일이 경과된 알림은 자동 배치 삭제
